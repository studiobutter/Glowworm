...
public sealed partial class ScreenshotSetting : PageBase
{
            ...
            if (e.HotkeyAvaliable)
            {
                ...
                else
                {
                    if (e.HotkeyChanged)
                    {
                        ...
                        {
                            InAppToast.MainWindow?.Warning(null, string.Format("", hotkey), 5000);
                        }
                        else
                        {
                            InAppToast.MainWindow?.Warning(null, string.Format("", hotkey), 5000);
    ...
    private async Task TestCaptureAsync()
    {
        try
        {
            ...
            Directory.CreateDirectory(Path.Combine(ScreenshotFolder, "Glowworm"));
            ...
            filePath = Path.Join(ScreenshotFolder, "Glowworm", $"Glowworm_{frameTime:yyyyMMdd_HHmmssff}.{extension}");
