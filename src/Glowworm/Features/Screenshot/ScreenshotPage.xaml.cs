...
public sealed partial class ScreenshotPage : PageBase
{
    ...
    public async Task<(string? BackupFolder, string? ScreenshotFolder)> GetGameScreenshotPathAsync()
    {
        try
        {
            ...
            string? installPath = GameRegistryHelper.GetGameInstallPath(CurrentGameBiz);
            if (installPath is not null && relativePath is not null)
            {
    ...
    public async Task<string?> GetCloudGameScreenshotPathAsync()
    {
        try
        {
            if (CurrentGameBiz.Game == GameBiz.hk4e)
            {
                ...
                if (File.Exists(configPath))
                {
                    ...
                    {
                        if (line.StartsWith("save_game_screenshot_directory="))
                        {
                            string folder = line.Substring("save_game_screenshot_directory=".Length).Trim();
                            if (Directory.Exists(folder))
                            {
                                return folder;
                            }
                ...
                string defaultFolder = Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.MyPictures), "GenshinImpactCloudGame");
            ...
            else if (CurrentGameBiz.Game == GameBiz.nap)
            {
                ...
                if (key != null)
                {
                    ...
                    {
                        string folder = Path.Join(installLocation, "ScreenShoot");
                        if (Directory.Exists(folder))
                        {
                            return folder;
                        }
        ...
        {
            _logger.LogError(ex, "Get cloud game screenshot path");
        }
        return null;
    }
        ...
        try
        {
            if (GridView_Images.SelectionMode is ListViewSelectionMode.Multiple && GridView_Images.SelectedItems.Count > 0)
            {
                ...
                if (list.Count > 0)
                {
                    ...
                    InAppToast.MainWindow?.Success("", string.Format("", list.Count), 1500);
                }
            }
            else if (sender is FrameworkElement fe && fe.DataContext is ScreenshotItem item)
            {
                if (File.Exists(item.FilePath))
                {
                    ...
                    InAppToast.MainWindow?.Success("", null, 1500);
                }
                else
                {
                    InAppToast.MainWindow?.Warning("", item.FilePath, 3000);
                }
            }
        }
        catch (Exception ex)
    ...
    private async void MenuFlyoutItem_CopyAsJPG_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            if (sender is FrameworkElement fe && fe.DataContext is ScreenshotItem item)
            {
                if (File.Exists(item.FilePath))
                {
                    ...
                    ClipboardHelper.SetStorageItems(DataPackageOperation.Copy, file);
                    InAppToast.MainWindow?.Success($"{""} ({GetSizeString(new FileInfo(jpgFilePath).Length)})", null, 1500);
                }
                else
                {
                    InAppToast.MainWindow?.Warning("", item.FilePath, 3000);
        ...
        {
            InAppToast.MainWindow?.Error("", ex.Message, 3000);
    ...
    private async void MenuFlyoutItem_Delete_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            // TODO ?? RPC ??
            InAppToast.MainWindow?.Warning("", "", 5000);
