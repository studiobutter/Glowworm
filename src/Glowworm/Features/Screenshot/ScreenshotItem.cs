...
public partial class ScreenshotItem : ObservableObject
{
    ...
    {
        // glowworm
        if (DateTime.TryParseExact(GlowwormNameRegex().Match(name).Value, "yyyyMMdd_HHmmssff", null, DateTimeStyles.None, out time))
        {
            return true;
        }
        // genshin zzz
    ...
    private static partial Regex GlowwormNameRegex();
