...
using ReleaseFile = Glowworm.Setup.Core.ReleaseFile;
using System;
...
internal class UpdateService
{
    ...
    private async Task StartInternalAsync(ReleaseInfoDetail release, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            string targetPath = Path.GetDirectoryName(AppConfig.GlowwormPortableLauncherExecutePath)!;
    ...
    private async Task PrepareForUpdateAsync(ReleaseManifest manifest, string targetPath, CancellationToken cancellationToken = default)
    {
        try
        {
            _logger.LogInformation("Prepare for update glowworm");
            State = UpdateState.Pending;
    ...
    private async Task UpdateAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _logger.LogInformation("Update Glowworm finished");
        }
        catch (OperationCanceledException)
        {
            _logger.LogInformation("Update Glowworm canceled, restore version.ini");
            State = UpdateState.Stop;
            RestoreVersionIni();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Update Glowworm failed, restore version.ini");
            State = UpdateState.Error;
            ErrorMessage = ex.Message;
            RestoreVersionIni();
        }
    ...
    private async Task DownloadFilesAsync(CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Update Glowworm, downloading {count} files, file size: {size}", _releaseManifest.DiffFileCount, Progress_TotalBytes);
        ...
        _logger.LogInformation("Update Glowworm, downloading files finished");
    }
    ...
    {
        _logger.LogInformation("Update Glowworm, patching and verifying files");
