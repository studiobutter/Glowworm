...
public class UninstallWindow : WindowBase
{
    ...
    public UninstallWindow() : base()
    {
        ...
        this.Title = Lang.GlowwormUninstaller;
        BuildUI();
    }
    ...
    private async void CheckUninstall()
    {
        try
        {
            ...
            string flag = Path.Combine(currentFolder!, "GlowwormInstallFolder");
            ...
            bool hasDatabase = files.Any(f => Path.GetFileName(f).Equals("GlowwormDatabase.db", StringComparison.OrdinalIgnoreCase));
            if (hasDatabase)
            {
                caution = true;
    ...
    private async void Uninstall()
    {
        try
        {
            ...
            await Task.Run(() =>
            {
                ...
                {
                    string cacheFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Glowworm");
    ...
    {
        string lnk = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "Glowworm.lnk");
        if (File.Exists(lnk))
        {
            File.Delete(lnk);
        }
