...
public class WindowBase : Window
{
    ...
    {
        var ps = Process.GetProcessesByName("Glowworm").Where(p => p.Id != Environment.ProcessId);
        if (ps.Any())
        {
            bool close = await MessageBox.AskYesNoAsync(string.Format(Lang.GlowwormIsRunningForceClose, string.Join(",", ps.Select(x => x.Id))), PromptIconKind.Warning, owner: this);
            if (close)
            {
                foreach (var p in ps)
                {
