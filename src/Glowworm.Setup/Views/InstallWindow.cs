...
using Glowworm.Setup.Locale;
using Glowworm.Setup.Services;
using Glowworm.Setup.Views;
using System.Diagnostics;
using WindowsShortcutFactory;

namespace Glowworm.Setup.Views;
...
public class InstallWindow : WindowBase
{
    ...
    private StackPanel BuildHeader()
    {
        return new StackPanel().Row(0)
            ...
            new Image().Center()
                       ...
                       .SourceResource<InstallWindow>("Glowworm.Setup.logo.png"),
    ...
    {
        base.OnLoaded();
        SetInstallFolder(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Glowworm"));
    ...
    private async Task PrepareManifestAsync(CancellationToken cancellation = default)
    {
        try
        {
            ...
            TextBlock_TitleHeader.Text = $"{Lang.GlowwormSetup}  �  {_installService.AppVersion}";
        }
        catch (Exception ex)
        {
            Debug.WriteLine(ex);
    ...
    private async void ChangeInstallFolder()
    {
        try
        {
            ...
            {
                if (new DirectoryInfo(folder).Name is not "Glowworm")
                {
                    folder = Path.Combine(folder, "Glowworm");
                }
    ...
    private async void StartInstall()
    {
        ...
        try
        {
            if (new DirectoryInfo(InstallFolder.Value).Name is not "Glowworm")
            {
                return;
            }
            ...
            await File.WriteAllTextAsync(Path.Combine(InstallFolder.Value, "GlowwormInstallFolder"), InstallFolder.Value);
            CreateShortcut();
    ...
    private void Launch()
    {
        try
        {
            string exe = Path.Combine(InstallFolder.Value, "Glowworm.exe");
            if (File.Exists(exe))
            {
                Process.Start("explorer", $"""
                    "{Path.Combine(InstallFolder.Value, "Glowworm.exe")}"
                    """);
                Environment.Exit(0);
            }
            else
    ...
    {
        CreateShortcutInternal(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "Glowworm.lnk"));
        string folder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Programs), "Glowworm");
        Directory.CreateDirectory(folder);
        CreateShortcutInternal(Path.Combine(folder, "Glowworm.lnk"));
    }
    ...
    private void CreateShortcutInternal(string shortcutPath)
    {
        ...
        {
            Path = Path.Combine(InstallFolder.Value, "Glowworm.exe"),
    ...
    private void OpenPrivacyPolicy()
    {
        ...
        {
            FileName = "https://github.com/Scighost/Glowworm/blob/main/docs/Privacy.md",
