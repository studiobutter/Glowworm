...
public abstract class GachaLogClient
{
    ...
    public static string GetGachaCacheFilePath(GameBiz gameBiz, string? installPath)
    {
        string file = gameBiz.Value switch
        {
            ...
            GameBiz.nap_cn or GameBiz.nap_global or GameBiz.nap_bilibili or GameBiz.nap_epic => Path.Join(installPath, WEB_CACHE_ZZZ_PATH),
        ...
        string prefix = gameBiz.Value switch
        {
            ...
            GameBiz.nap_cn or GameBiz.nap_global or GameBiz.nap_bilibili or GameBiz.nap_epic => @"ZenlessZoneZero_Data\webCaches",
            _ => throw new ArgumentOutOfRangeException($"Unknown region {gameBiz}"),
        };
    ...
    private static ReadOnlySpan<byte> GetGachaUrlPattern(GameBiz gameBiz)
    {
        return gameBiz.Value switch
        {
            ...
            GameBiz.nap_global or GameBiz.nap_epic => SPAN_WEB_PREFIX_ZZZ_OS,
            _ => throw new ArgumentOutOfRangeException($"Unknown region {gameBiz}"),
        };
    }
    ...
    protected virtual async Task<List<T>> GetGachaLogByTypeAsync<T>(string prefix, IGachaType gachaType, long endId = 0, IProgress<(IGachaType GachaType, int Page)>? progress = null, CancellationToken cancellationToken = default) where T : GachaLogItem
    {
    ...
    public async Task<ZZZGachaWiki> GetZZZGachaInfoAsync(GameBiz gameBiz, string lang, CancellationToken cancellationToken = default)
    {
        ...
        if (gameBiz.IsChinaServer() && lang is "zh-cn")
        {
            const string url = $"https://starward-static.scighost.com/metadata/v1/zzz/ZZZGachaInfo.nap_cn.zh-cn.json"; // DO NOT CHANGE
            wiki = await CommonGetAsync<ZZZGachaWiki>(url, cancellationToken);
        }
        else
        {
            string url = $"https://starward-static.scighost.com/metadata/v1/zzz/ZZZGachaInfo.nap_global.{lang}.json"; // DO NOT CHANGE
            wiki = await CommonGetAsync<ZZZGachaWiki>(url, cancellationToken);
        }
